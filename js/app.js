    ...
    var SURF_SPOTS = [
      ...
      { name: 'Paco de Arcos',    lat: 38.6905, lon: -9.2966, scale: 0.45, camUrl: 'https://video-auth1.iol.pt/beachcam/parcos/playlist.m3u8' },
      { name: 'Praia de Caxias',  lat: 38.6987, lon: -9.2792, scale: 0.45 },
      { name: 'São João da Caparica', lat: 38.6562, lon: -9.2501, scale: 0.50, camUrl: 'https://video-auth1.iol.pt/beachcam/bccostasaojoao/playlist.m3u8' },
      { name: 'Costa da Caparica', lat: 38.6449, lon: -9.2419, scale: 0.50, camUrl: 'https://video-auth1.iol.pt/beachcam/bccaparicanovapraia/playlist.m3u8' },
      { name: 'Fonte da Telha',   lat: 38.5733, lon: -9.1969, scale: 0.50, camUrl: 'https://video-auth1.iol.pt/beachcam/fontedatelha/playlist.m3u8' },
      { name: 'Bicas',            lat: 38.4637, lon: -9.1929, scale: 0.50, camUrl: 'https://video-auth1.iol.pt/beachcam/bcpraiadasbicas/playlist.m3u8' },
    ...
    function updateSpotCardConditions(idx, wh, period, ws, wd, swellH, whMin, whMax) {
      ...
      if (windEl) {
        if (ws != null && wd != null) {
          ...
          var glowColor = wColor.replace('1.0)', '0.35)');
          windEl.innerHTML =
            '<span class="sp-wind-arrow" style="transform:rotate(' + (wd + 180) + 'deg);color:' + wColor + ';text-shadow:0 0 14px ' + glowColor + ';">↑</span>'
            + '<span class="sp-wind-type" style="color:' + wColor + ';text-shadow:0 0 12px ' + glowColor + ';">' + typeLabel + '</span>'
            + '<span class="sp-wind-spd">' + dirLabel + ' · ' + Math.round(ws) + ' km/h</span>';
        } else {
          windEl.innerHTML = '';
        }
      }
    ...
    (function buildSpotCards() {
      ...
      SURF_SPOTS.forEach(function(spot, idx) {
        ...
        card.innerHTML =
          ...
          + (spot.name === 'Praia Pequena' ? '<img src="Pequena Sticker.png" class="sp-card-sticker sp-card-sticker--pequena" alt="">' : '')
